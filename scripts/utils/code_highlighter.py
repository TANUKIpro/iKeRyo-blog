        ...
        if parts:
            ...
            for part in parts[1:]:
                if ':' in part:
                    ...
                    if style.lower() in self.LINE_STYLES:
                        line_numbers = self._parse_line_ranges(ranges)
                        if line_numbers:
                            line_highlights[style.lower()] = line_numbers
        ...
        for range_part in ranges_str.split(','):
            ...
            if '-' in range_part:
                ...
                try:
                    ...
                    self.logger.warning(f"無効な行範囲: {range_part}")
            else:
                # 単一行
                try:
                    line_numbers.append(int(range_part))
                except ValueError:
                    self.logger.warning(f"無効な行番号: {range_part}")
        
        return sorted(set(line_numbers))  # 重複を除去してソート
    
    def _create_line_html(self, line_number: int, line_content: str, line_highlights: Dict[str, List[int]]) -> str:
        """単一行のHTMLを生成"""
        # この行に適用されるスタイルを検索
        applied_styles = []
        ...
        # 行のHTML生成
        if applied_styles:
            # 最初に見つかったスタイルを適用（優先順位: error > warning > success > info > highlight > add > remove）
            style_priority = ['error', 'warning', 'success', 'info', 'highlight', 'add', 'remove']
            selected_style = None
            for style in style_priority:
                if style in applied_styles:
